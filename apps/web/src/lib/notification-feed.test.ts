...
import type { AccountNotification } from "./api";
import {
...
function line(createdAt: string, id = createdAt): AccountNotification {
  return { id, kind: "ORDER_PAID", title: "سفارش پرداخت شد", body: null, href: null, createdAt };
}
...
describe("readMarkerStorageKey", () => {
  it("keys the marker by customer so a shared browser never mixes two accounts", () => {
    expect(readMarkerStorageKey("BP-100")).not.toBe(readMarkerStorageKey("BP-200"));
  });
});
...
describe("readStoredMarker", () => {
  ...
  it("treats a value that is not a date as no marker at all", () => {
    const restore = withStorage({ getItem: () => "yesterday" });
...
describe("writeStoredMarker", () => {
  it("swallows a storage failure rather than breaking the panel", () => {
    const restore = withStorage({
      setItem: () => {
...
describe("unreadCount", () => {
  ...
  /* Without this, an existing customer's first visit would show a badge over
   * every order they already know about. */
